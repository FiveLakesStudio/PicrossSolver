...
void init_line(Puzzle *puz)
{
     ...
     */
    lpos= (line_t *)realloc(lpos, (maxnclue + 1) * sizeof(line_t));
    rpos= (line_t *)realloc(rpos, (maxnclue + 1) * sizeof(line_t));
    gcov= (line_t *)realloc(gcov, maxnclue * sizeof(int));
    
    /* An extra color bit map for apply_lro */
    oldval= (bit_type*)realloc(oldval, fbit_size * sizeof(bit_type));
    
    col= (bit_type *)realloc(col, maxdimension * fbit_size * sizeof(bit_type));
    if (puz->ncolor > 2)
        nbcolor= (line_t *)realloc(nbcolor, puz->ncolor * sizeof(line_t));
}
