...
int picrossHDSolveCommandLineEntry(int argc, char **argv)
{
    char *filename= NULL;
    Puzzle *puz;
...
#define MAXWATCH 20
    ...
    else if (terse)
    {
        ...
        {
            printf(maybacktrack ? "contradiction " : "stalled ");
        }
        else if (rc)
        {
            if (isunique)
            {
                ...
                {
                    if (contrafound > 0)
                        printf("unique depth-%d ",contradepth);    /* !! TC !! Removed \n and added a space*/
                    else
                        printf("unique logical ");
                }
                else
                    printf("unique ");
            }
